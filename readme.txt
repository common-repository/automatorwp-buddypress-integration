=== AutomatorWP - BuddyPress integration ===
Contributors: automatorwp, rubengc, eneribs
Tags: buddypress, automatorwp, automator, automation, forum, topic, reply
Requires at least: 4.4
Tested up to: 5.9
Stable tag: 1.1.2
License: GNU AGPLv3
License URI: http://www.gnu.org/licenses/agpl-3.0.html

Connect AutomatorWP with BuddyPress

== Description ==

> **Important:** Now all free integrations are included in [AutomatorWP](https://wordpress.org/plugins/automatorwp/ "AutomatorWP") so you no longer need to install them one by one!
>
> We will continue working to improve AutomatorWP and make it easier, faster and more accessible to everyone.

[BuddyPress](https://wordpress.org/plugins/buddypress/ "BuddyPress") helps you build any kind of community website using WordPress, with member profiles, activity streams, user groups, messaging, and more.

This plugin automatically connects [AutomatorWP](https://wordpress.org/plugins/automatorwp/ "AutomatorWP") with BuddyPress adding new triggers and actions that you can use to connect with other plugins and automate workflows letting you save time and get focused on your most important work.

= Triggers =

* User account gets activated.
* User updates avatar.
* User updates cover image.
* User updates profile information.
* User writes an activity stream message.
* User joins any/specific group.

= Actions =

* Set user profile type.
* Add an activity to the user.

= Pro version =

[Pro version](https://automatorwp.com/add-ons/buddypress/ "AutomatorWP - BuddyPress") available that let's you expand the integration between AutomatorWP and BuddyPress bringing you more ways to automatize your community.

= What is AutomatorWP? =

AutomatorWP is an automator plugin that lets you connect your WordPress plugins together and add automation to unlimited workflows.

You can automate anything including sales, marketing, administrative tasks, learning and any other kind of processes you want letting you save time and get focused on your most important work.

= More integrations =

WordPress.org is home to some amazing integrations for AutomatorWP, including:

* [WooCommerce](https://wordpress.org/plugins/automatorwp-woocommerce-integration/)
* [LearnDash](https://wordpress.org/plugins/automatorwp-learndash-integration/)
* [H5P](https://wordpress.org/plugins/automatorwp-h5p-integration/)
* [WP Fusion](https://wordpress.org/plugins/automatorwp-wp-fusion-integration/)
* [MemberPress](https://wordpress.org/plugins/automatorwp-memberpress-integration/)

== Installation ==

= From WordPress backend =

1. Navigate to Plugins -> Add new.
2. Click the button "Upload Plugin" next to "Add plugins" title.
3. Upload the downloaded zip file and activate it.

= Direct upload =

1. Upload the downloaded zip file into your `wp-content/plugins/` folder.
2. Unzip the uploaded zip file.
3. Navigate to Plugins menu on your WordPress admin area.
4. Activate this plugin.

== Frequently Asked Questions ==

== Screenshots ==

== Changelog ==

= 1.1.2 =

* **Improvements**
* Prevent use of undefined constant

= 1.1.1 =

* **New Features**
* Added new tags on triggers based on groups with the group information.

= 1.1.0 =

* **New Features**
* Added support to the AutomatorWP custom values feature.

= 1.0.9 =

* **New Features**
* Added new activity tags to pass the activity ID and URL.
* Added a new tag to pass the activity comment content.

= 1.0.8 =

* **New Features**
* New action: Add user to group.

= 1.0.7 =

* **Bug Fixes**
* Fixed profile type selector.

= 1.0.6 =

* **New Features**
* Added new tags to pass the activity content.

= 1.0.5 =

* **New Features**
* New trigger: User updates avatar.
* New trigger: User updates cover image.
* New trigger: Set user profile type.
* New action: Set user profile type.
* New action: Add an activity to the user.

= 1.0.4 =

* **Bug Fixes**
* Fixed group ID checks.

= 1.0.3 =

* **Bug Fixes**
* Fixed group ID checks.

= 1.0.2 =

* **Bug Fixes**
* Fixed typo on integration icon URL.

= 1.0.1 =

* **Bug Fixes**
* Fixed incorrect file name.

= 1.0.0 =

* Initial release.
