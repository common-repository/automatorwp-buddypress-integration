...
class AutomatorWP_BuddyPress_Add_User_Activity extends AutomatorWP_Integration_Action {
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Add an activity to the user', 'automatorwp-buddypress' ),
            'select_option'     => __( 'Add an <strong>activity</strong> to the user', 'automatorwp-buddypress' ),
            /* translators: %1$s: Activity. */
            'edit_label'        => sprintf( __( 'Add an %1$s to the user', 'automatorwp-buddypress' ), '{activity}' ),
            /* translators: %1$s: Activity. */
            'log_label'         => sprintf( __( 'Add an %1$s to the user', 'automatorwp-buddypress' ), '{activity}' ),
            'options'           => array(
                'activity' => array(
                    'default' => __( 'activity', 'automatorwp-buddypress' ),
                    'fields' => array(
                        // Note: "action" key is reserved
                        'activity_action' => array(
                            'name' => __( 'Action:', 'automatorwp-buddypress' ),
                            'desc' => __( 'Activity\'s action.', 'automatorwp-buddypress' ),
                            'type' => 'text',
                            'default' => ''
                        ),
                        'content' => array(
                            'name' => __( 'Content:', 'automatorwp-buddypress' ),
                            'desc' => __( 'Activity\'s content.', 'automatorwp-buddypress' ),
                            'type' => 'wysiwyg',
                            'default' => ''
                        ),
                        'link' => array(
                            'name' => __( 'Link:', 'automatorwp-buddypress' ),
                            'desc' => __( 'URL link associated to the activity.', 'automatorwp-buddypress' ),
                            'type' => 'text',
                            'default' => ''
                        ),
                        'hide_sitewide' => array(
