...
class AutomatorWP_BuddyPress_Add_User_Group extends AutomatorWP_Integration_Action {
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Add user to group', 'automatorwp-buddypress' ),
            'select_option'     => __( 'Add user to <strong>group</strong>', 'automatorwp-buddypress' ),
            /* translators: %1$s: Group. */
            'edit_label'        => sprintf( __( 'Add user to %1$s', 'automatorwp-buddypress' ), '{group}' ),
            /* translators: %1$s: Group. */
            'log_label'         => sprintf( __( 'Add user to %1$s', 'automatorwp-buddypress' ), '{group}' ),
            'options'           => array(
                'group' => automatorwp_utilities_ajax_selector_option( array(
                    'field'             => 'group',
                    'option_default'    => __( 'group', 'automatorwp-buddypress' ),
                    'name'              => __( 'Group:', 'automatorwp-buddypress' ),
                    'option_none'       => false,
                    'option_custom'         => true,
                    'option_custom_desc'    => __( 'Group ID', 'automatorwp-buddypress' ),
                    'action_cb'         => 'automatorwp_buddypress_get_groups',
                    'options_cb'        => 'automatorwp_buddypress_options_cb_group',
                    'placeholder'       => __( 'Select a group', 'automatorwp-buddypress' ),
                    'default'           => '',
                ) )
            ),
        ) );
